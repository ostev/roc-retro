...
update = \snake, gamepad ->
    direction = 
        if gamepad.buttons.dpad.left then {x: -1, y: 0}
            else if gamepad.buttons.dpad.right then {x: 1, y: 0}
            else if gamepad.buttons.dpad.up then {x: 0, y: -1}
            else if gamepad.buttons.dpad.down then {x: 0, y: 1}
            else snake.direction
    
    newHead = when List.first snake.body is
